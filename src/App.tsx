...
import { EVOLUTIONS, PACKS, CARDS_DB, ALL_CARDS, ABOMINATIONS_DB } from './constants';
...
const SvgPointer = ({ size = 150, color = "currentColor" }) => (<svg width={size} height={size} viewBox="0 0 100 100" fill="none" stroke={color} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M30 20 L70 60 L50 65 L60 90 L45 95 L35 70 L20 80 Z" /></svg>);
const SvgStitchFinger = ({ size = 150, color = "currentColor" }) => (<svg width={size} height={size} viewBox="0 0 100 100" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"><path d="M40 90 L40 30 C40 10, 60 10, 60 30 L60 90" /><path d="M35 40 L65 45 M35 60 L65 55 M35 75 L65 80" strokeWidth="4" /><circle cx="50" cy="25" r="3" fill={color} /></svg>);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', position: 'relative', padding: 20 }} onClickCapture={tryInitAudio}>
      ...
      >
        ...
        <div style={{ padding: '0 10px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <button className="btn" onPointerDown={e => e.stopPropagation()} onClick={() => updateState({ collection: { 1: 3 }, money: 0 })}>DEV: 3 GLOOMS</button>
      ...
      <header className="hylics-panel" style={headerStyle}>
        <div style={{ padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h1 className="title" style={{ margin: 0, fontSize: '2.5rem' }}>HOLLOW·SOMNIUM</h1>
        ...
        <div style={{ padding: '30px', overflowY: 'auto', height: '100%' }}>
                ...
                <div style={{
                  ...
                  >
                    <div style={{ pointerEvents: 'none' }}>
                      {state.clickLevel === 0 && <SvgPointer color={currentEvolution.color} />}
                      {state.clickLevel === 1 && <SvgStitchFinger color={currentEvolution.color} />}
                      {state.clickLevel === 2 && <SvgMindScraper color={currentEvolution.color} />}
                      {state.clickLevel === 3 && <SvgGloomHand color={currentEvolution.color} />}
                    </div>
                    <div style={{ ...coinInnerStyle, textShadow: '2px 2px 0 var(--hylics-border-dark)' }}>
          ...
          {activeTab === 'shop' && (
            <div style={shopLayout}>
              {PACKS.map(pack => (
                <div key={pack.id} className="hylics-panel" style={packCardStyle}>
                  <div style={{ ...packArtStyle, background: pack.color, color: 'black' }}>{pack.name}</div>
                  <div style={{ padding: 15, display: 'flex', flexDirection: 'column' }}>
                    <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => buyPack(pack)} disabled={state.money < pack.price}>
                      PURCHASE <br />({pack.price.toLocaleString()})
                    </button>
                  </div>
                </div>
              ))}
                ...
                {ABOMINATIONS_DB.map(abom => {
                  ...
                  return (
                    <div key={abom.id} className="hylics-panel" style={{ width: 280, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <h3 style={{ color: 'var(--accent-primary)', textShadow: '2px 2px 0 black' }}>{abom.name}</h3>
...
const packCardStyle: React.CSSProperties = { width: 240, display: 'flex', flexDirection: 'column', overflow: 'hidden' };
const packArtStyle: React.CSSProperties = { width: '100%', height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '1.5rem', textShadow: '1px 1px 0px white', borderBottom: '4px solid var(--hylics-border-white)' };
